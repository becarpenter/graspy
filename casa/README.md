...
There are also three binary files in the `casa` filespace:
 ...
 - `casa_apdls.bin`: the tokens that have been used to identify APADLs generated by the CASA. These are retained for audit purposes.
...
A known weakness is that the code does not keep a full record of which tokens have been assigned to which APDL, so tokens cannot be invalidated when an APADL is compromised or lost. At the moment this is left as future work.
