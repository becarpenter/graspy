...
`casa_setup.py` - various utilities and declarations used by the other components (except `casa-proxy.py`). It also defines a few configuration values including the port used for EST operations (currently set to 443).
...
In the `casa` filespace, the certificates of pledges that have
successfully onboarded are stored in the form `<token>.pem` where
`token` is the relevant token as a hexadecimal number, e.g.
`6ff287cb20402271.pem`.

There are also three binary files in the `casa` filespace:

 - `casa_tokens.bin`: all the tokens ever generated by this CASA, in chronological order.

 - `casa_flags.bin`: the corresponding flag bytes for all those tokens, in the same order. The flag bits are defined in `casa_setup.py`.

 - `casa_apdls.bin`: the tokens that have been used to identify APADLs generated by the CASA. These are retained for audit purposes, and could be used to expunge pledge certificates for lost APADLs (no code implemented for that yet).

For more details, see the `casa.py` source file.

In the APDL filespace (i.e. a memory stick) there are two binary files:

 - `atokens.bin`: same format as `casa_tokens.bin`, but only contains the tokens assigned to this APDL.

 - `aflags.bin`: same format as `casa_flags.bin`, but only for the tokens assigned to this APDL.

For more details, see the `casa-installer.py` source file.
...
A known weakness is that the code does not keep a full record of which tokens have been assigned to which APDL, so tokens cannot be invalidated when an APADL is compromised or lost.
